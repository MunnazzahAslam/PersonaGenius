...
export default function Page() {
  ...
  const options = {
    filename: "persona-genius-results.pdf",
    ...
    <>{response && <>
      ...
      {showPdfContent && (
        <div id="container" className="m-4">
          <Typography variant="h4" className="section-heading">Cluster Summaries</Typography>
          {response.cluster_summaries.map((summary, index) => (
            <Card key={index}>
              <CardHeader>
          ...
          <Typography variant="h4" className="section-heading">Cluster Personas</Typography>
          {response.cluster_personas.map((persona, index) => (
            <div key={index}>
              <div className="flex justify-start items-center my-12">
                <h2 className="text-2xl font-bold mt-8 mb-4">{persona.demographics.name}</h2>
              </div>
              {persona?.demographics && <Card>
                <CardHeader>
                  <CardTitle>Demographic Data</CardTitle>
